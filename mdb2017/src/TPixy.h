...
struct Block 
{
    ...
    if (signature>PIXY_MAX_SIGNATURE) // color code! (CC)
  {
      ...
      float pOffXCenter = (x - PIXY_CENT_X) / PIXY_CENT_X;
      float pOffYCenter = (y - PIXY_CENT_Y) / PIXY_CENT_Y;

      sprintf(buf, "CC block! sig: %s (%d decimal) x: %d y: %d width: %d height: %d angle %d\n", sig, signature, x, y, width, height, angle);
    }     
  else // regular block.  Note, angle is always zero, so no need to print
  {
      float pOffXCenter = ((float) (x - PIXY_CENT_X)) / (float) PIXY_CENT_X;
      float pOffYCenter = ((float) (y - PIXY_CENT_Y)) / (float) PIXY_CENT_Y;
      sprintf(buf, "sig: %d x: %d y: %d width: %d height: %d Percent off X: %f Off Y: %f\n", signature, x, y, width, height,pOffXCenter,pOffYCenter);
  }
      printf(buf);
  }
  uint16_t signature;
